...
// g++ -std=c++17 -O2 -Wall -Wextra  transmitNoise.cpp -o noise   $(pkg-config --cflags --libs uhd) -lboost_system -lboost_thread -pthread
// ./noise continuous
// constjaming continuous --noise --samp-rate 1e6 --freq 2437e6 --gain 170
...
// ---------- GLOBAL CONFIG ----------
std::string TCP_TARGET_IP = "127.0.0.1";
int TCP_TARGET_PORT = 5000;

// Global listening socket
int g_server_fd = -1;

bool init_tcp_listener()
{
    ...
    addr.sin_port = htons(TCP_TARGET_PORT);
    ...
    std::cout << "[TCP] Listening on port " << TCP_TARGET_PORT << "...\n";
    return true;
}
std::string wait_for_tcp_trigger()
{
    std::cout << "[TCP] Waiting for trigger...\n";

    sockaddr_in client{};
    socklen_t csz = sizeof(client);

    // ONLY accept() here
    int client_fd = accept(g_server_fd, (struct sockaddr *)&client, &csz);
    if (client_fd < 0)
    {
        perror("accept");
        return "";
...
int main(int argc, char *argv[])
{
    ...
    //auto ist_zone = std::chrono::locate_zone("Asia/Kolkata");
    ...
    try
    {
        ...
        if (cfg.mode == "continuous")
        {
            ...
            while (true)
            {
                ...
                std::string trigger_msg = wait_for_tcp_trigger();
                    // Get current UTC time
                //std::string trigger_msg    
                auto now = system_clock::now();

                // IST = UTC + 5h 30m
                auto ist = now + hours(5) + minutes(30);

                // Convert to time_t
                std::time_t t = system_clock::to_time_t(ist);

                // Convert to struct tm (UTC base)
                ...
                // std::cout << "[TCP] Trigger received: \"" << trigger_msg << "\"\n";
                
                double burst_ms = 0.003; // 3 ms burst
                auto t0 = std::chrono::steady_clock::now();

                std::cout << "[TX] Sending noise burst...\n";

                // Keep sending until duration elapsed
                while (std::chrono::duration<double>(std::chrono::steady_clock::now() - t0).count() < burst_ms)
                {
                    tx_stream->send(&buf.front(), buf.size(), md, 1.0);
                }

                std::cout << "[TX] Burst complete. Waiting for next trigger...\n";
            }
        }
        else
        {
