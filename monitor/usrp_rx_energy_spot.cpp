// usrp_rx_energy_spot.cpp
// Receive-only instantaneous energy detector using UHD
// Compile with: g++ -std=c++17 usrp_rx_energy_spot.cpp -luhd -lboost_system -o usrp_rx_energy_spot
...
int main(int argc, char* argv[]){
    ...
    for(int i=1;i<argc;i++){
        std::string s = argv[i];
        if(s == "--samp-rate" && i+1<argc) samp_rate = atof(argv[++i]);
        else if(s == "--freq" && i+1<argc) center_freq = atof(argv[++i]);
        else if(s == "--gain" && i+1<argc) gain = atof(argv[++i]);
        else if(s == "--args" && i+1<argc) args = argv[++i];
        else if(s == "--out" && i+1<argc) out_file = argv[++i];
        else if(s == "--buf" && i+1<argc) buffer_len = (size_t)atoi(argv[++i]);
        else if(s == "--thresh-db" && i+1<argc) thresh_db = atof(argv[++i]);
        else if(s == "--debounce-ms" && i+1<argc) debounce_ms = atol(argv[++i]);
        else if(s == "--mode" && i+1<argc) mode = argv[++i];
        else if(s == "--help"){
    ...
    try{
        ...
        if(!out_file.empty()){
            ...
            if(!fout) { std::cerr << "Failed to open output file: " << out_file << "\n"; out_file.clear(); }
            else std::cout << "Writing captured IQ to: " << out_file << "\n";
        }
